...
Was there something weird happening in 2012?

```{r load_2012}
d12 = read_tsv('pde_date_counts_2012.tsv') %>%
  mutate(date=dmy(date)) %>%
  mutate(week_n=week(date)) %>%
  mutate(day_of_week=date %>% weekdays %>% factor(levels=dow_levels, ordered=TRUE)) %>%
  group_by(day_of_week) %>%
...
bind_rows(d11 %>% mutate(data_year=2011),
          d12 %>% mutate(data_year=2012)) %>%
  mutate(data_year=factor(data_year)) %>%
  ggplot(aes(x=date, y=count, color=data_year, group=data_year)) +
  geom_point() + geom_line()
```