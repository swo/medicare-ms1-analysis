...
state_consumption = read_tsv('../../bene-consumption/state-consumption-2011.tsv') %>%
  rename(consumption_drug=drug) %>%
  select(state, consumption_drug, claims_per_1k_ppl, did)
...
```{r run_models, echo=FALSE, warning=FALSE}
...
```{r make_plots, echo=FALSE, message=FALSE}
unlink('plots/*.pdf')
...
res %>%
  ...
  select(bug, consumption_drug, resistance_drug,
         state_coherent, explanatory, correlation, p.value,
         plot_link) %>%
  kable
```