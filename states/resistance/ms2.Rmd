...
I pooled the antiobiograms from all years.
...
models = function(df, independent_var, explanatory_vars) {
  f = paste0(independent_var, ' ~ ', paste0(explanatory_vars, collapse = ' + ')) %>% formula
  lm(f, data=df) %>% tidy
}

models_result = function(df) {
  group_by(df, bug, drug_group, place) %>%
    summarize(mean_percent_nonsusceptible=weighted.mean(mean_percent_nonsusceptible, place_weight),
              cpkp=unique(cpkp),
              did=unique(did)) %>%
    group_by(bug, drug_group) %>%
    do(models(., 'mean_percent_nonsusceptible', 'cpkp')) %>%
    ungroup %>%
    filter(term=='cpkp') %>%
...
swo> Copy the code, and allow a function pointer for lm function to use
