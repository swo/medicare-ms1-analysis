...
- Implement weights in the linear correlations
...
safe_mblm = function(data, indep, expl) {
  try(return(mblm(sprintf('%s ~ %s', indep, expl), data) %>% tidy), silent=TRUE)
  data_frame(term=expl, estimate=NA, std.error=NA, statistic=NA, p.value=1.0)
}

ts_models = function(df, indep) {
  lapply(c('cpkp', 'did'), function(expl) {
    safe_mblm(df, indep, expl) %>%
      filter(term==expl) %>%
      rename(explanatory=term) %>%
      verify(nrow(.) == 1)
  }) %>% bind_rows
...
ts_models_result = function(x, this_place) {
  x %>%
    group_by_('bug', 'drug_group', this_place) %>%
    summarize(mean_nonsus=mean(percent_nonsusceptible, na.rm=TRUE),
              cpkp=head(cpkp, 1),
...
ts_state_res = linear_models_result(state_dat, 'state')
