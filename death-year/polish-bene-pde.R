...
# This is mostly the same as the polish PDE file in the above directory, but
# here I allow less than 12 months of coverage in the case that the person died
...
polish_bene = function(x) {
  rename(x, bene_id=BENE_ID,
    ...
    mutate(death_date=dmy(DEATH_DT)) %>%
    filter(!is.na(state),
           plan_coverage_months > 0, hmo_months==0,
           plan_coverage_months==12 | !is.na(death_date)) %>% # filter for geography and plan
    select(bene_id, death_date, sex, age, race, state, zipcode, plan_coverage_months)
}
