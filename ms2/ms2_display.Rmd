...
The gray line shows $\beta_{f_>} = 1$.
...
Why should the linear models do so much better than the Spearman? I think it's because, in the linear model, I can weight the geographical units that have more antibiograms. The Spearman model treats them all equally.
...
Rather than mean consumption $\mu$, I'll use the mean consumption among consumers $\mu_> = \mu / f_>$.
...
We ask if mean nonzero consumption $\mu_>$ has any information beyond fraction nonzero $f_>$ for predicting resistance $R$. We might expect that the simple two-part model represented by the regressions $R \sim f_>$ doesn't capture the fact that taking two doses should have a greater effect than taking one, etc.

These plots show the coefficient $\beta_{\mu_>}$ from the regression $R^{(u)} = \beta_0 + \beta_{f_>} f_>^{(u)} + \beta_{\mu_>} \mu_>^{(u)} + \varepsilon^{(u)}$.

```{r hrr_multi}
results %>%
  filter(model=='multivariate', unit_type=='hrr', term=='mup') %>%
  interval_plot() +
  ylab(expression(beta[mu['>']])) +
  ggtitle("Linear multivariate, HRR")
```

At the state level, none of the $\beta_{\mu_>}$ coefficients are significantly different from zero (after multiple-hypothesis correction).

```{r state_multi}
results %>%
  filter(model=='multivariate', unit_type=='state', term=='mup') %>%
  interval_plot() +
  ylab(expression(beta[mu['>']])) +
...
- *E. faecium* and $\beta$-lactams have a postive relationship (implying higher mean consumption, given the same fraction of people consuming, goes with more resistance)
- *S. pneumoniae* and tetracyclines have a negative relationship

For *E. faecium* with $\beta$-lactams, the univariate regression with $f_>$ is not significant, and the one with $\mu_>$ is marginally so. Similarly, for *S. pneumoniae* and tetracyclines, the regression with $f_>$ is not significant, and the regression wiht $\mu_>$ is. Thus, it looks like $\mu_>$ isn't adding information in addition to $f_>$; instead, it just has all the information.

And some marginal results:

- *E. coli* and $\beta$-lactams, negative relationship, HRR only; significant in univariate regressions $R \sim f_>$
- *E. cloacae* and quinolones, positive, state only; marginally significant in univariate
- Non-MRSA *S. aureus* and first-generation cephalosporins, negative, HRR only; but not significant in univariate
- *E. faecium* and nitro, negative, both HRR and state; not significant in univariate
- *S. aureus* and clinda, positive, state only; not significant in univariate
- *S. aureus* and tetracyclines, positive, state only; not significant in univariate

### E. coli and Bactrim

This negative relationship is the most tricky.

**swo double-check** First, the regression results are equivocal. Regressing $r ~ \mu$, where $r$ are the residuals from the regression $R ~ f_>$, gives a non-significant result. An ANOVA analysis shows that sequential application of $f_>$ and $\mu$ as predictors of $R$ both explain non-trivial variance; but if the order is reversed, and $\mu$ is used first, then it *doesn't* explain any variance. This makes me think that there is just some weird interaction between the two terms in the multiple regression.

Second, from the Spearman and univariate models, Bactrim resistance in *E. coli* is correlated with increased resistance (black line). When breaking that apart by region, however, there are different relationship (colored lines). This makes it seem like there is something strange going on.
...
Only the West has a significantly non-zero slope.

### E. coli & Bactrim, Part II

Weirdly, within each region, Bactrim resistance goes does as $\mu_>$ increases.
...
hrr_ir %>%
  filter(bug=='E. coli', drug_group=='tmp_smx') %>%
  filter(mean_percent_nonsusceptible < 50) %>% # filter one crazy Nevada point
  ggplot(aes(x=mup, y=mean_percent_nonsusceptible, color=region)) +
  geom_smooth(method='lm', se=F, aes(color=NULL), color='black') +
  geom_smooth(method='lm', se=F) +
  geom_point() +
...
Here, the South and West have significantly non-zero slopes.

At least this explains, in a sense, why this relationship looks negative: the South is perched up high, so overall the black line goes down.
...
Not surprisingly, linear regressions $R \sim \mu_>$ give slightly different results from $R \sim f_>$. The negative correlations are all in those drugs where $\mu_>$ was anticorrelated with $f_>$.
