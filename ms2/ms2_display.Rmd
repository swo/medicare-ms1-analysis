...
The plot shows the Spearman correlation coefficient $\rho$ between resistance
$R$ and mean consumption $\mu$. Two stars after the bug/drug combination
indicates that the adjusted $p < 0.05$, and one star means the unadjusted $p <
0.05$. Intervals show 95% confidence intervals.
...
```{r hrr_linear_fnz_table}
results %>%
  filter(model=='univariate_fnz', unit_type=='hrr', term=='fnz') %>%
  select(bug, drug_group, estimate, anova.p.value) %>%
...
results %>%
  ...
  arrange(desc(estimate)) %>%
  # swo: fanciful now
  left_join(ineq_hrr_mean, by='drug_group') %>%
  mutate(dR_nu_ext=-(mup-1)/fnz*estimate,
         dR_nu_int=estimate/fnz) %>%
  select(bug, drug_group, estimate, anova.p.value, dR_nu_ext, dR_nu_int) %>%
  kable
```
...
```{r both_effects}
results %>%
  filter(unit_type=='hrr', model=='multivariate_fnz_mup', term != '(Intercept)') %>%
  select(bug, drug_group, term, estimate) %>%
  spread(term, estimate) %>%
  rename(beta_fnz=fnz, beta_mup=mup) %>%
  left_join(ineq_hrr_mean, by='drug_group') %>%
  mutate(dR_ext=beta_fnz - (mup - 1)/fnz * beta_mup,
         dR_int=beta_mup / fnz) %>%
  kable
```
...
```{r pneumo_macro_minimal}
inner_join(
  ineq %>%
    filter(unit_type=='state', drug_group=='macrolide') %>%
    select(state=unit, fnz),
  state_abg %>%
    filter(bug=='S. pneumoniae', drug_group=='macrolide') %>%
    select(state, percent_nonsusceptible),
by='state') %>%
  left_join(regions, by='state') %>%
  ggplot(aes(x=fnz, y=percent_nonsusceptible, label=state, color=region)) +
  geom_smooth(method='lm', se=F, col='gray') +
  geom_point() +
  #geom_text() +
  ggrepel::geom_text_repel() +
  xlab(str_interp('fraction f of beneficiaries with a macrolide claim')) +
  ylab(str_interp('percent nonsusceptible pneumococcus isolates')) +
  theme_classic()
```
