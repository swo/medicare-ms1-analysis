...
The gray line shows $\beta_{f_>} = 1$. Most confidence intervals include that line, i.e., we can explain our data with a transmission-model where individuals who take drug have 100% resistance and others have some smaller amount.
...
Interestingly, these are the same three values that are significant when performing the (ill-advised) regression using $f_>$ and $\mu$:

```{r fnz_mu_results}
results %>%
  filter(unit_type=='hrr', model=='multivariate_fnz_mup', term=='mup') %>%
  filter(p.adjust(p.value, method='BH') < 0.05) %>%
  select(bug, drug_group, estimate, p.value) %>%
