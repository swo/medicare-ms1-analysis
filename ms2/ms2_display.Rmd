...
ineq = read_tsv('ineq.tsv') %>%
  ...
  summarize_at(vars(mean, fnz), mean) %>%
  ungroup() %>%
  mutate(mup=mean/fnz)
...
The plot shows the Spearman correlation coefficient $\rho$ between resistance $R$ and fraction $f_>$ of beneficiaries who are consumers. Two stars after the bug/drug combination implies that the adjusted $p < 0.05$, and one star means the unadjusted $p < 0.05$. Intervals show 95% confidence intervals. 
...
The tetracyclines and Bactrim are the interesting exceptions: in these drugs, states where consumption is more widespread are the states where consumption per consumer is lower.
