...
```{r}
results %>%
  filter(model=='univariate_mean', unit_type=='hrr', term=='mean') %>%
  interval_plot() +
  geom_hline(yintercept=1, col='gray') +
  ylab(expression(beta[f])) +
  ggtitle("Linear univariate, HRR")
...
```{r hrr_multi}
results %>%
  filter(model=='multivariate', unit_type=='hrr', term=='mean') %>%
  interval_plot() +
...
```{r state_multi}
results %>%
  filter(model=='multivariate', unit_type=='state', term=='mean') %>%
  interval_plot() +
  ylab(expression(beta[mu])) +
...
The second two, *E. faecium* with $\beta$-lactams and pneumococcus with tetracyclines, are not problematic: in both cases, neither $R \sim f_>$ nor $R \sim \mu$ is significant. We're only interested in cases where $f_>$ gives information and $\mu$ gives information beyond that. There are cases where $f_>$ doesn't appear to give any information.
...
How do the regressions $R \sim f_>$ compare to $R \sim \mu$? Here I look at the correlation between the coefficients $\beta_\mu$ and $\beta_{f_>}$ across bug/drug combinations. The correlation is really high for HRRs and just regular high for states.
...
results %>%
  ...
  group_by(unit_type) %>%
  summarize(cor_between_betas=cor(fnz, mean)) %>%
  kable
```
