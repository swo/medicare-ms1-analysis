...
ineq %>%
  ...
  #rename(state=unit) %>% left_join(regions, by='state') %>%
...
### Effect along extensive margin $\mu_>$
...
We ask if mean consumption $\mu$ has any information beyond fraction nonzero $f_>$ for predicting resistance $R$. We might expect that the simple two-part model (i.e., consuming or not) represented by the regressions $R \sim f_>$ doesn't capture the fact that taking two doses should have a greater effect than taking one, etc.

These plots show the coefficient $\beta_\nu$ from the regression $R = \beta_0 + \beta_{f_>} f_> + \beta_\nu \nu + \varepsilon$.
...
results %>%
  ...
  interval_plot() +
  ylab(expression(beta[nu])) +
  ggtitle("Linear multivariate (f> and nu), HRR")
```

At the state level, none of the $\beta_\nu$ coefficients are significantly different from zero (after multiple-hypothesis correction).

```{r state_multi}
results %>%
  filter(model=='multivariate_fnz_mup', unit_type=='state', term=='mup') %>%
  interval_plot() +
  ylab(expression(beta[nu])) +
  ggtitle("Linear multivariate (f> and nu), state")
```
...
```{r fnz_mu_results}
results %>%
  filter(unit_type=='hrr', model %in% c('univariate_fnz', 'univariate_mup', 'multivariate_fnz_mean', 'multivariate_fnz_mup'), term != '(Intercept)') %>%
  (function(df) {
...
I contrast this situation against *E. coli* and quinolones. In that case, adding terms to the regression doesn't drastically alter $\beta_{f_>}$.
...
ineq %>%
  ...
  right_join(state_abg, by=c('state', 'drug_group')) %>%
  filter(bug=='E. coli', drug_group=='beta_lactam') %>%
  filter(percent_nonsusceptible < 50) %>% # filter one crazy point
  ggplot(aes(x=mean, y=percent_nonsusceptible, color=region)) +
  geom_smooth(method='lm', se=F, aes(color=NULL), color='black') +
  geom_smooth(method='lm', se=F) +
  geom_point() +
...
```{r univariate_result_correlation, eval=FALSE}
results %>%
...
We don't expect that, over a large range, the consumption/resistance relationship will be linear. We expect it would be something more like a logistic function. To evaluate the difference between these two approaches, I performed quasibinomial regressions $R \sim f_>$. If $\bar{f}$ is the midpoint of the $f_>$ values (i.e., the weighted mean) and $\hat{y}$ is the predicted resistance from the model, then the slope at the midpoint is $\beta_f \hat{y}(\bar{x}) \left[ 1- \hat{y}(\bar{x}) \right]$. I compare the $p$-values and slopes in the linear and quasibinomial regressions:

```{r linear_quasibin}
results %>%
  filter(unit_type=='hrr', model %in% c('univariate_fnz', 'univariate_fnz_log'), term=='fnz') %>%
  select(bug, drug_group, model, estimate, p.value) %>%
  group_by(model) %>%
  mutate(p.value = p.adjust(p.value, method='BH')) %>%
  ungroup() %>%
  ggplot(aes(x=estimate, y=-log10(p.value), color=model, group=interaction(bug, drug_group))) +
  geom_hline(yintercept = -log10(0.05)) +
  geom_point() +
  geom_line(color='gray')
```

Points above the black line are significant. Note that the $p$-values in the two cases are really similar (i.e., most dumbbells are on the same horizontal line) but they can differ somewhat in their slope. The right-most pink dot is pneumococcus/macrolides, and the next is *P. mirabilis*/Bactrim.

I also tried doing beta regressions, which get similar-sized changes in the slope but really significant $p$-values. Not sure what to make of those.

```{r linear_beta}
results %>%
  filter(unit_type=='hrr', model %in% c('univariate_fnz', 'univariate_fnz_beta'), term=='fnz') %>%
  select(bug, drug_group, model, estimate, p.value) %>%
  group_by(model) %>%
  mutate(p.value = p.adjust(p.value, method='BH')) %>%
  ungroup() %>%
