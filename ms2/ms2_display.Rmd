...
```{r hrr_linear}
results %>%
  filter(model=='univariate', unit_type=='hrr', term=='fnz') %>%
  interval_plot() +
...
```{r state_linear}
results %>%
  filter(model=='univariate', unit_type=='state', term=='fnz') %>%
  interval_plot() +
  ylab(expression(beta[f])) +
...
- *E. coli* and $\beta$-lactams, negative relationship, HRR only
- Non-MRSA *S. aureus* and first-generation cephalosporins, negative, HRR only
- *E. faecium* and nitro, negative, both HRR and state
- *S. aureus* and clinda, positive, state only
- *S. aureus* and tetracyclines, positive, state only
- *E. cloacae* and quinolones, positive, state only

The positive relationships are not troubling: it means that it might be that increasing consumption among the consumers can increase resistance. The negative relationships are troubling: how could decreasing consumption among consumers increase resistance?

### E. faecium and beta-lactams

This one doesn't worry me. First, the univariate regression $R \sim f_>$ isn't significant. Second, the ANOVA gives different results depending on the order of the variables: if $\mu$ goes first, it is non-significant but $f_>$, going second, is; conversely, if $f_>$ goes first, then $\mu$ is significant. So I think this is a weird case about collinearity.

```{r efaecium_betalactams}
ineq %>%
  ...
  ylab('Beta-lactam resistance E. faecium') +
...
From the Spearman and univariate models, Bactrim resistance in *E. coli* is correlated with increased resistance (black line). When breaking that apart by region, however, there are different relationship (colored lines).
...
This one doesn't bother me either. Tetracycline resistance in pneumococcus not significantly related ot tetracycline consumption in the univariate models; the coefficient is almost exactly zero. If you run a regression $R \sim \mu$, you get a non-significant result. So you only get this relationship *after* you correct for an approximately flat relationship. Not promising.
...
ineq %>%
  ...
  xlab('fraction who consumed Bactrim') +
  ylab('Bactrim-resistance E. coli') +
  ggtitle('S. pneumonia & Bactrim across HRRs')
```
