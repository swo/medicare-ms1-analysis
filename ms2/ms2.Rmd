...
# min_n_states = 35
# good_combos = semi_join(raw_abg, possible_bug_drug, by=c('bug', 'drug_group')) %>%
#   select(bug, drug_group, state) %>%
#   distinct %>%
#   count(bug, drug_group) %>% ungroup() %>% 
#   rename(n_places=n) %>%
...
abg = raw_abg %>%
  ...
  #right_join(good_combos, by=c('bug', 'drug_group')) %>%
...
# get the number of states in which each bug/drug appears
min_n_states = 37
abg_n_states = abg %>%
  select(bug, drug_group, state) %>%
  distinct() %>%
...
abg %<>%
  ...
  left_join(abg_n_states, by=c('bug', 'drug_group')) %>%
  left_join(abg_n_hrr, by=c('bug', 'drug_group'))
  #filter(n_places >= min_n_states)
...
multi_term_adjuster = 'cpb'
multi_term = 'nb_prob' # multivariable explanatory term (along with fnz)
models = function(x) {
...
In previous iterations, I filtered by the number of states in which each bug/drug combination appeared. Increasing that threshold ends up testing the relationships for which we have the best data, and therefore we are the best powered for. Unfortunately, my simulations mostly make it seem like we should, to get the best power, use all the bug/drug combinations.
...
```{r, eval=FALSE}
# Used this to show that there's a correlation between
# strength of correlation and amount of data
x = abg %>% 
  select(bug, drug_group, state) %>%
  distinct() %>%
  count(bug, drug_group) %>% ungroup() %>% rename(n_places=n)
results %>% filter(model=='spearman') %>%
  left_join(x, by=c('bug', 'drug_group')) %>%
