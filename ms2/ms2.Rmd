...
I similarly joined the consumption drugs into similar classes. I'm looking at these `r nrow(good_combos)` bug/drug combinations:
...
```{r spearman_models, cache=FALSE, eval=FALSE}
results %>%
  filter(model=='spearman') %>%
  mutate(p.adj=p.adjust(p.value, method='BH')) %>%
  #filter(p.adj < 0.05) %>%
  arrange(p.adj) %>%
  select(bug, drug_group, estimate, p.adj, n_data) %>%
  kable
```
...
```{r univariate_models, cache=FALSE, eval=FALSE}
results %>%
...
```{r multivariate_models, cache=FALSE, eval=FALSE}
results %>%
...
ecoli_bactrim_dat = left_join(state_abg, state_ineq, by=c('state', 'drug_group')) %>%
  left_join(regions, by='state') %>%
  filter(bug=='Escherichia coli', drug_group=='tmp_smx') %>%
