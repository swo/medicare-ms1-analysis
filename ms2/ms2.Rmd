...
**swo**: filter the NAs in the abg earlier
...
all_ineq = read_tsv('data/state_ineq_all_2011.tsv') %>%
  left_join(regions, by='state')
state_ineq = read_tsv('data/state_ineq_2011.tsv')
hrr_ineq = read_tsv('data/hrr_ineq_2011.tsv')
```
...
results %>%
  ...
  #filter(p.adj < 0.05) %>%
  arrange(p.adj) %>%
  select(bug, drug_group, estimate, p.value, p.adj, n_data) %>%
  kable
```
...
And what about looking at multivariate only for ones that passed the threshold for univariate? Doesn't seem like it works for any of them!
...
```{r}
left_join(hrr_abg, hrr_ineq, by=c('hrr', 'drug_group')) %>%
  left_join(hrr, by='hrr') %>%
  filter(bug=='E. coli', drug_group=='quinolone') %>%
  ggplot(aes(x=mean, y=mean_percent_nonsusceptible, color=region, size=sqrt(total))) +
  geom_smooth(aes(x=mean, y=mean_percent_nonsusceptible, color=NULL), method='lm', col='black') +
  geom_point() +
  xlab('quinolone claims per beneficiary') + ylab('mean percent nonsusceptible E. coli isolates') +
  theme_minimal()
...
## E.coli and penicillin

```{r}
ecoli_pen_dat = left_join(state_abg, state_ineq, by=c('state', 'drug_group')) %>%
  left_join(regions, by='state') %>%
  filter(bug=='E. coli', drug_group=='penicillin') %>%
  filter(!is.na(mean_percent_nonsusceptible))
