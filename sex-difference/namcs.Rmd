...
visits = read_tsv_years('../../db/namcs/data/namcs_visits_%i.tsv') %>%
  ...
  replace_na(list(uti=FALSE, acne=FALSE)) %>%
...
I kept the drugs from azithromycin through clarithromycin, accounting for 80% of all prescriptions.
...
female_visit_odds = x %>% filter(sex=='female') %$% odds

x %>%
  select(sex, `fraction of visits`=mean, SE, odds) %>%
  kable
```

Overall, more of the visits were made by women. Women make `r round(female_visit_odds - 1, 2)*100`% more visits than men. I'll therefore evaluate consumption by two standards: first, do women consume more antibiotics overall; second, do women consume more antibiotics *per visit*.
...
abx_total_f = function(abx) {
  svyby(as.formula(paste0('~', abx)), ~sex, adult_design, svytotal) %>%
    rename_(.dots=setNames(list(abx), c('total'))) %>%
    select(sex, total, se) %>%
    mutate(abx=abx)
}

poisson_f = function(abx) {
  svyglm(as.formula(paste0(abx, '~sex')), adult_design) %>%
    tidy %>%
...
abx_totals %>%
  ...
  select(-sex) %>%
  spread(key, value) %>%
  mutate(highlight=abx %in% c('nitro', 'clinda')) %>%
  ggplot(aes(x=x, xmin=xmin, xmax=xmax, y=y, ymin=ymin, ymax=ymax, label=abx, color=highlight)) +
  geom_abline(col='gray') + geom_abline(slope=female_visit_odds, col='gray') +
  geom_point() + geom_errorbar() + geom_errorbarh() +
  scale_color_manual(values=c('black', 'red')) +
  geom_text() +
  #geom_text(nudge_x=1, nudge_y=0.5) +
  #scale_x_continuous(breaks=c(0, 5, 10), limits=c(0, 12)) + xlab('No. rx. for men (millions)') +
  #scale_y_continuous(breaks=c(0, 5, 10), limits=c(0, 14)) + ylab('No. rx. for women (millions)') +
  coord_fixed() +
  theme_minimal()
```
...
abx_diff %>%
  ...
  mutate(sig=case_when(.$adj.p.value < 0.05 ~ '**',
                       .$p.value < 0.05 ~ '*',
                       TRUE ~ '')) %>%
...
abx_pois %>%
  ...
  mutate_at(vars(-abx), scientific_round) %>%
  kable
```
...
```{r clinda}
m1 = svyglm(clinda~sex, design=adult_design, family='poisson')
m2 = svyglm(clinda~sex+acne, design=adult_design, family='poisson')
clinda_f_expl = 1 - exp(-m2$coefficients['sexmale']+m1$coefficients['sexmale'])
```

The only clindamycin-associated diagnosis that meets (maybe) the data validity criterion is acne (`7061-`). However, acne explains only a small fraction (`r 100*round(clinda_f_expl, digits=3)`%) of the difference between the sexes.
