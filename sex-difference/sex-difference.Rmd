...
ubs = lapply(top_abx$antibiotic, usage_by_sex) %>% bind_rows
```
...
ubs %>%
  ...
  geom_abline() +
  #geom_abline(slope=c(1.06, 1.109, 1.366, 1.41, 2.202), col='gray') +
  #geom_abline(slope=c(1.062406, 1.0976, 1+0.26*1.268177), col='blue') +
  geom_abline(slope=exp(3)*(1.06)) +
  # NAMCS: all adult usage 1.495, UTI: 2.164, ARC: 1.0976
  # Medicare: nabxclaims 1.24, n OP days 1.062406, arc 1.108944, uti 2.268177, copd MALE! 1.123728
  geom_text() +
  geom_point() +
...
dx %>%
  ...
  group_by(sex) %>%
  summarize(mn=mean(n), n_bene=n())
```

```{r drug_by_sex_plot2, eval=FALSE}
total_plot_f = function(df, highlight=character(0)) {
  df %>%
    ...
    mutate(ci=se*1.96,
           ci_low=total-ci,
           ci_high=total+ci) %>%
    select(drug, sex, total, ci_low, ci_high) %>%
    gather('key', 'value', total, ci_low, ci_high) %>%
    ...
    select(-sex) %>%
    spread(key, value) %>%
    mutate(highlight=drug %in% highlight) %>%
    ggplot(aes(x=x, xmin=xmin, xmax=xmax, y=y, ymin=ymin, ymax=ymax, label=drug, color=highlight)) +
...
ubs %>%
  gather('sex', 'total', female, male) %>%
  left_join(bene %>% count(sex) %>% rename(n_bene=n), by='sex') %>%
  mutate(se=total/sqrt(n_bene)) %>%
  rename(drug=antibiotic) %>%
  total_plot_f() +
  geom_abline(slope=c(1.06, 1.306122), col='gray')
```
