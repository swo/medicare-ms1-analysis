...
dx_codes = read_tsv('../../data/fd_codes.tsv') %>%
  rename(dx=code, desc=dx_desc)

diagnosis_types = unique(dx_codes$diagnosis_type)

dx = read_tsv_years('../../db/namcs/data/namcs_dx_%i.tsv') %>%
  left_join(dx_codes, by='dx')
...
## Diagnoses across the sexes

What's the difference in primary diagnoses across the sexes? I look in not-children.

```{r diagnoses}
unique_primary_dx = dx %>%
  filter(dx_number==1) %>%
  left_join(visits, by=c('visit_id', 'year')) %>%
...
```{r}
f = function(code) {
  filter(dx, dx_number==1, dx==code) %>%
    mutate(n_dx=1) %>%
    left_join(visits, by=c('visit_id', 'year')) %>%
    replace_na(list(n_dx=0)) %>%
    design_f() %>%
    subset(age_group != 'child') %>%
    svyby(~n_dx, ~sex, design=., svytotal) %>%
    as_data_frame() %>%
    mutate(code=code)
}

dx_by_sex = lapply(unique_primary_dx, f) %>% bind_rows()
dx_by_sex
```
...
dx_by_sex %>%
  rename(n=n_dx) %>%
  mutate(ci=1.96*se, ci_low=n-ci, ci_high=n+ci) %>%
  select(code, sex, n, ci_low, ci_high) %>%
  gather('key', 'value', n:ci_high) %>%
  mutate(key=case_when(.$sex=='male' & .$key=='n' ~ 'x',
  ...
  select(-sex) %>%
  spread(key, value) %>%
  ggplot(aes(x=x, xmin=xmin, xmax=xmax, y=y, ymin=ymin, ymax=ymax, label=code)) +
  geom_abline(col='gray') +
  geom_point() + geom_errorbar() + geom_errorbarh() + geom_text() +
  coord_fixed() +
  theme_minimal()
```

```{r}
g = function(diagnosis_type) {
  filter(dx, dx_number==1, diagnosis_type==diagnosis_type) %>%
    mutate(n_dx=1) %>%
    left_join(visits, by=c('visit_id', 'year')) %>%
    replace_na(list(n_dx=0)) %>%
    design_f() %>%
    ...
    svyby(~n_dx, ~sex, design=., svytotal) %>%
    as_data_frame() %>%
    mutate(diagnosis_type=diagnosis_type)
}

#dt_by_sex = lapply(diagnosis_types, f) %>% bind_rows()
#dt_by_sex
g('pneumonia')
```
...
How many antibiotic prescriptions total were made by each sex?

```{r overall_abx}
svyby(~n_abx, ~sex, svytotal, design=adult_design)
```
