...
bene = read_tsv(sprintf('../bene_%i.tsv', this_year)) %>%
  filter(age >= 65, age < 116, hmo_months==0) %>%
...
usage = pde %>% count(bene_id) %>% rename(n_claims=n) %>%
  ...
  mutate(age_over_65 = age - 65)
  
stopifnot(all(usage$age >= 0))
```

# All-condition negative binomial model

Each point represents a chronic condition. The coordinates are corresponding coefficients in the two models. The formula for the model is claims against age and sex.

```{r all_condition_negbin}
all_condition_formula_start = 'n_claims ~ age_over_65 + is_female + '
all_condition_formula = formula(paste(c(all_condition_formula_start, conditions), collapse=' + '))

all_condition_negbin_model = usage %>%
  MASS::glm.nb(all_condition_formula, data=.) %>%
  tidy()

all_condition_poisson_model = usage %>%
  glm(all_condition_formula, data=., family='poisson') %>%
  tidy()
```

```{r cache=FALSE}
bind_rows(mutate(all_condition_poisson_model, method='Poisson'),
          mutate(all_condition_negbin_model, method='negbin')) %>%
  ...
  xlab('Poisson multivariate model coefficient') +
  ylab('negative binomial multivariate model coefficient') +
  coord_fixed() +
  theme_minimal()
```
...
rename_(x, .dots=setNames(condition, 'has_condition')) %>%
  glm(n_claims ~ age_over_65 + is_female + has_condition, family='poisson', data=.) %>%
  tidy() %>%
  mutate(condition=condition)
}
  
single_condition_models = lapply(conditions, function(cond) summarize_condition(usage, cond)) %>%
  bind_rows()
```
...
lasso_x = usage %>%
  select(age_over_65, is_female, AMI:HYPOTH) %>%
  as.matrix
...
race_geography_model_f = function(condition) {
  form = formula(paste0('n_claims ~ age_over_65 + is_female + is_white + region + ', condition))
  usage %>%
