...
dx_categories = read_tsv('../../data/fd_codes.tsv') %>%
  mutate(dx=str_pad(code, 5, 'right', '-')) %>%
  select(dx, diagnosis_type)
unique_dx_categories = unique(dx_categories$diagnosis_type)

dx = read_tsv_years('../../db/namcs/data/namcs_dx_%i.tsv') %>%
  left_join(dx_categories, by='dx')

dx_visit = dx %>%
  filter(!is.na(diagnosis_type)) %>%
  mutate(dummy=TRUE) %>%
  select(visit_id, diagnosis_type, dummy) %>%
  distinct() %>%
...
visits = read_tsv_years('../../db/namcs/data/namcs_visits_%i.tsv') %>%
  ...
  mutate_at(top_abx$abx, function(x) {
    case_when(is.na(x) ~ 0L,
              x == 0 ~ 0L,
              x > 0 ~ 1L)
  }) %>%
  mutate_at(unique_dx_categories, function(x) if_else(is.na(x), FALSE, x))
```
...
bind_rows(x, y) %>%
  add_pct_change() %>%
  mutate(abx=factor(abx, levels=abx_levels),
         abx=forcats::fct_relevel(abx, 'overall', after=Inf)) %>%
...
per_capita %>%
  filter(term=='year') %>%
  mutate(valid=abs(std.error/estimate)<=0.3,
...
show_var = function(df) sprintf('%g (±%g)', round(df$apc*1000, 1), round(df$se*1000, 1))
```
...
The NAMCS data can actually confirm that, not only is levo going up per visit, it's going up faster in those with URIs (and not for pneumonia). (Here I'm looking at the entire population.)

```{r levo_dx}
svyglm(levo ~ year*uri + year*pneumonia, design=design, family='poisson') %>%
  tidy %>%
  kable
```
...
We can also confirm that keflex is going up because of UTIs and now because of SSTIs (even though the use of keflex for UTIs is so much smaller than for SSTIs):

```{r keflex_dx}
svyglm(keflex ~ year*ssti + year*uti, design=design, family='poisson') %>%
  tidy %>%
  kable
```
