...
pde_inapprop_risk_2011 = pde_approp_table %>%
  # add up tiers 3 and 4
...
adjusted_changes_firstfill = read_tsv('tables/model_abx_firstfill.tsv') %>%
  ...
  arrange(pct_change) %>%
  mutate(antibiotic=fct_relevel(fct_inorder(factor(antibiotic)), 'overall', after=Inf))

# compute risk ratios
pde_inapprop_trends = read_tsv('tables/pde_inapprop_trends.tsv') %>%
  filter(term=='year', antibiotic != 'Other') %>%
  mutate(estimate=exp(3*estimate), # odds ratio over the whole time period
         ...
         ci_low=estimate-ci,
         ci_high=estimate+ci) %>%
  left_join(pde_inapprop_risk_2011, by='antibiotic') %>%
  ...
  select(antibiotic, model, pct_change=estimate, ci_low, ci_high, adj.p) %>%
  # now estimate is adjusted change in INappropriate prescribing
  mutate(antibiotic=factor(antibiotic, levels=levels(adjusted_changes_firstfill$antibiotic), ordered=TRUE))
```
...
adjusted_changes %>%
  ...
  bind_rows(pde_inapprop_trends %>%
              filter(antibiotic=='overall', model=='12/3') %>%
              mutate(antibiotic='overall_inapp')) %>%
  mutate(antibiotic=factor(antibiotic, levels=c(adjusted_change_levels, 'overall_inapp', 'overall'))) %>%
...
```{r}
ggplot(adjusted_changes_firstfill, aes(x=antibiotic, y=pct_change, ymin=ci_low, ymax=ci_high, color=model)) +
  geom_hline(yintercept=0) + 
  geom_bar(aes(color=NULL), stat='identity', fill=NA, color='black') +
  geom_errorbar(aes(color=NULL), width=0.5) +
  geom_point(dat=pde_inapprop_trends, shape=1) +
  geom_errorbar(dat=pde_inapprop_trends, width=0.25) +
  coord_flip() +
  xlab('') +
...
```{r}
bind_rows(
  unadjusted_changes %>% mutate(src='unadjusted, all fills'),
  adjusted_changes %>% mutate(src='adjusted, all fills'),
  adjusted_changes_firstfill %>% mutate(src='adjusted, first fills')
) %>%
  mutate(antibiotic=factor(antibiotic, levels=c(adjusted_change_levels, 'overall'))) %>%
  ggplot(aes(x=antibiotic, y=pct_change, ymin=ci_low, ymax=ci_high, color=src)) +
  geom_hline(yintercept=0, color='black') +
  geom_point(shape=1) +
  geom_errorbar(width=0.5) +
  coord_flip() +
  xlab('') + ylab('change from 2011 to 2014 (%)') +
  theme_minimal()
```
