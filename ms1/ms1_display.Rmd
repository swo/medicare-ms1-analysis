...
drug_trends = read_tsv('tables/new/abx_models.tsv') %>%
...
combined_table = rxdx_table %>%
  ...
  mutate_at(vars(estimate, lower_ci, upper_ci), est2pct) %>%
  mutate(change=sprintf('%.1f (%.1f -- %.1f)', estimate, lower_ci, upper_ci)) %>%
  mutate(d=pmax(estimate-lower_ci, upper_ci-estimate)) %>%
  mutate(change2=sprintf('%.1f ± %.1f', estimate, d)) %>%
  mutate(p.value=format_p(p_value)) %>%
  select(dx_cat, antibiotic, `2011`, `2014`, change2, p.value) %>%
...
rxdx_kable(c('azithromycin', 'levofloxacin', 'amoxclav'),
           c('pneumonia', 'sinusitis', 'uri', 'bronchitis'))
```
...
```{r other_respiratory}
rxdx_kable(c('azithromycin', 'levofloxacin', 'amoxclav'),
           c('pharyngitis', 'asthma', 'other_resp'))
```
