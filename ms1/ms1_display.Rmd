...
adjusted_az_decrease = x %>%
  filter(antibiotic=='azithromycin') %$%
  { (1 + pct_change/100)^3 } %>%
  { (1 - .) * 100 } %>%
  round(2)

adjusted_levo_increase = x %>%
...
dx_counts = read_tsv('tables/dx_counts.tsv') %>%
  ...
  mutate(dx_pkb=n/n_bene*1000) %>%
  select(diagnosis_type, year, dx_pkb)

dx_counts_trends = dx_counts %>%
  group_by(diagnosis_type) %>%
  arrange(year) %>%
  summarize(p=trend_p(dx_pkb))

dx_counts %>%
  spread(year, dx_pkb) %>%
  mutate(pct_change_per_year=(`2014`-`2011`)/`2011`*100/3) %>%
  left_join(dx_counts_trends, by='diagnosis_type') %>%
  arrange(desc(`2011`)) %>%
  mutate_at(vars(`2011`:p), function(x) round(x, 2)) %>%
  kable
```

I also performed a similar analysis using logistic regression. I predicted whether a beneficiary would have a diagnosis at any point in the year. The main predictor variable was year, and the covariates were as elsewhere.

```{r adjusted_dx_or}
read_tsv('tables/model_dx.tsv') %>%
  filter(term=='year0') %>%
  model_ci() %>%
  select(diagnosis_type, change_str) %>%
  kable
```
