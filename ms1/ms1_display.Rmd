...
drug_trends = read_tsv('raw_data/poisson_models.tsv') %>%
  select(abx=model, term=Parameter, estimate=Estimate, std.error=StdErr, p.value=ProbChiSq) %>%
  filter(term=='year') %>%
  arrange(estimate) %>%
  ...
  mutate(y=est2pct(estimate),
         ymin=est2pct(ci_lo),
         ymax=est2pct(ci_hi)) %>%
  mutate(change=sprintf('%.1f (%.1f -- %.1f)', y, ymin, ymax))
```
...
drug_trends %>%
  ...
  mutate_at(vars(y, ymin, ymax), function(x) format(x, digits=1, nsmall=1)) %>%
...
```{r odds}
inapp_odds_trend = read_tsv('raw_data/logistic_models.tsv') %>%
  select(model, term=Variable, estimate=Estimate, std.error=StdErr, p.value=ProbChiSq) %>%
  filter(term=='year') %>%
  mutate(estimate=3*estimate,
         ci=1.96*sqrt(3)*std.error,
         y=exp(estimate),
         ymin=exp(estimate-ci),
         ymax=exp(estimate+ci)) %>%
  select(model, y, ymin, ymax) %>%
  mutate_if(is.numeric, function(x) round((x-1)*100, 1))

inapp_odds_trend %>% kable
```
...
```{r combined_table}
format_pct = function(x) format(round(x*100, 1), digits=1, nsmall=1)
