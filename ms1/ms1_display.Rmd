...
```{r tier_graph}
pde_tier_table = dx_pde_counts %>%
  filter(has_dx_em) %>%
  
pde_tier_table = read_tsv('raw_data/dx_pde_counts.tsv') %>%
  group_by(year, 
...
adjusted_changes %>%
  ...
  mutate(antibiotic=factor(antibiotic, levels=rev(adjusted_change_levels))) %>%
...
The bars show the adjusted change in consumption.
