...
First, unadjusted trends in the number of diagnoses per 1,000 beneficiaries in each year:

```{r dx_counts}
read_tsv('tables/dx_counts.tsv') %>%
...
```{r dx_trends_adjusted}
read_tsv('tables/dx_models.tsv') %>%
  filter(term=='year0') %>%
  mutate(pct_change=100*(exp(estimate*3)-1),
         p.value=p.adjust(p.value, method='BH')) %>%
  mutate_if(is.numeric, function(x) round(x, 2)) %>%
  select(diagnosis_category, pct_change, p.value) %>%
  kable
```

## Common diagnoses by drug

For each drug, I asked what fraction of PDEs are associated with each diagnosis. This gives you a sense that, say, the diagnoses that give rise to azithromycin are bronchitis, URI, etc. The values are percents. I might display just the 2011 data in the paper.

```{r pde_dx}
read_tsv('tables/pde_dx_counts.tsv') %>%
  filter(antibiotic %in% top_abx, year %in% c(2011, 2014)) %>%
  rename(pct=f_pde_with_dx) %>%
  select(antibiotic, year, diagnosis_category, pct) %>%
  spread(year, pct) %>%
  mutate(pct_change=`2014`/`2011`-1) %>%
  gather('key', 'value', c('2011', '2014', 'pct_change')) %>%
...
For each diagnosis and each drug, I ask what fraction of diagnoses are followed by a PDE for that drug.

```{r dx_pde}
read_tsv('tables/dx_pde_counts.tsv') %>%
  filter(antibiotic %in% top_abx, year %in% c(2011, 2014)) %>%
  rename(pct=f_dx_with_pde) %>%
  select(antibiotic, year, diagnosis_category, pct) %>%
  spread(year, pct) %>%
  mutate(pct_change=`2014`/`2011`-1) %>%
  gather('key', 'value', c('2011', '2014', 'pct_change')) %>%
...
These are logistic regressions. The unit of analysis is the diagnosis. The outcome is whether a PDE for that drug follows a diagnosis in that category. The main predictor is year; covariates are as above. The odds ratio is between 2011 and 2014. I'll convert that into a risk ratio using the 2011 probabilities from the previous table.

```{r dx_pde_adjusted}
read_tsv('tables/dx_pde_model.tsv') %>%
  filter(term=='year0') %>%
  mutate(p.value=p.adjust(p.value, method='BH'),
         OR=round(exp(3*estimate), 2)) %>%
  filter(p.value < 0.05) %>%
  select(antibiotic, diagnosis_category, OR) %>%
  spread(diagnosis_category, OR) %>%
  head %>%
  kable
```
