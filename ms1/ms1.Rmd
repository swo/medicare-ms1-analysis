...
single_condition_model = function(x, condition) {
  ...
  rename_(.dots=setNames(condition, 'has_condition')) %>%
    glm(n_claims ~ age + is_female + has_condition,
...
analyze = function(year) {
  ...
  claims_by_age = dat$bene %>%
    bind_rows(. %>% mutate(group='all'),
              . %>% filter(n_cc==0) %>% mutate(group='no_cc'),
...
claims_by_age %>%
  filter(year==2011, between(age, 67, 96)) %>%
  ggplot(aes(x=age, y=mean_n_cc)) +
  geom_point(shape=1, size=6) +
  geom_smooth(method='lm', se=FALSE, col='black') +
  theme_minimal(base_size=24) +
  scale_x_continuous(breaks=c(67, 70, 80, 90, 96)) +
  ylab('mean no. chronic conditions') +
  ylim(0, 6)
```
