...
```{r analyze_function}
age_group_f = function(x) bounds_group(x, c(65, 70, 75, 80, 85, 90, 95, 100))
...
analyze = function(year) {
  ...
  claims_by_age = dat$usage %>%
    ...
    summarize(n_bene=n(),
              ...
              mean_n_claims_0cc=mean(n_claims[n_cc==0]),
              sd_n_claims_0cc=sd(n_claims[n_cc==0]),
              mean_n_cc=mean(n_cc),
              sd_n_cc=sd(n_cc)) %>%
    mutate(sem_n_claims=sd_n_claims/sqrt(n_bene),
           sem_n_claims_0cc=sd_n_claims_0cc/sqrt(n_bene),
           sem_n_cc=sd_n_cc/sqrt(n_bene))
  
  chars_by_age_group = dat$usage %>%
...
f = function(df) {
  stopifnot(nrow(df) == 1)
  data_frame(group=c('all', '0cc'),
             mean_n_claims=c(df$mean_n_claims, df$mean_n_claims_0cc),
             sem_n_claims=c(df$sem_n_claims, df$sem_n_claims_0cc))
}

claims_by_age %>%
  filter(year==2011, between(age, 66, 96)) %>%
  select(age, mean_n_claims, mean_n_claims_0cc) %>%
  gather('group', 'mean_n_claims', mean_n_claims, mean_n_claims_0cc) %>%
  ggplot(aes(x=age,
             y=mean_n_claims,
             color=group,
             group=group)) +
