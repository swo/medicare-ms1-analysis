...
multivariate_formula = paste(c('n_claims ~ age_over_65 + is_female + ', conditions), collapse=' + ') %>% formula
...
analyze = function(this_year) {
  ...
  # keep only PDEs for known beneficiaries (semi join)
  # convert all amox/clav to just amox
  pde = read_tsv(sprintf('../pde_%i.tsv', this_year)) %>%
    semi_join(bene, by='bene_id') #%>%
    #mutate(antibiotic=if_else(antibiotic=='amoxicillin/clavulanate', 'amoxicillin', antibiotic))
  ...
  # not just by raw age)
  age_group_f = function(x) bounds_group(x, c(65, 70, 75, 80, 85, 90, 95, 116))
