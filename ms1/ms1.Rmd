...
- Remove visits from the models, since we expect that visits are casually in between disease and antibiotics
- Put all years together, put calendar year into the model, and regress (rather than doing separate regressions on each year)
...
analyze = function(year) {
  ...
  condition_prevalence = dat$bene %>%
    select(AMI:HYPOTH) %>%
    summarize_all(mean) %>%
    gather('condition', 'prevalence', everything())
...
```{r arrange_condition_prevalence}
condition_prevalence = data_by_field('condition_prevalence') %>%
