...
analyze = function(year) {
  ...
  chars_by_age_group = dat$usage %>%
    ...
    group_by(age_group) %>%
    summarize(mean_n_claims=mean(n_claims), mean_n_cc=mean(n_cc)) %>%
    mutate(cpkp=mean_n_claims*1000) %>%
    select(age_group, cpkp, mean_n_cc)
...
chars_by_age_group = data_by_field('claims_by_age_group') %>%
  mutate(cpkp=round(cpkp), mean_n_cc=round(mean_n_cc, digits=2)) %>%
  select(age_group, year, cpkp, mean_n_cc) %>%
  gather('variable', 'value', cpkp, mean_n_cc) %>%
  mutate(variable=interaction(year, variable)) %>%
  select(-year) %>%
  spread(variable, value) %T>%
